...
from utils.tools import get_configs_of, to_device, synth_samples, synth_samples_savepred
...
def synthesize(device, model, args, configs, vocoder, batchs, control_values):
    ...
    for batch in batchs:
        ...
        with torch.no_grad():
            ...
            output = model(
                *(batch[:-5]),
                spker_embeds=batch[-5],
                emotions=batch[-4],
                history_info=batch[-3],
                emphasis_data_batch=batch[-2],
                text_words_tod_emb_lens=batch[-1],
                p_control=pitch_control,
                e_control=energy_control,
                d_control=duration_control
            )
            
            synth_samples_savepred(
                batch,
                output[0],
                vocoder,
                model_config,
...
if __name__ == "__main__":
    ...
    if args.mode == "single":
        ...
        speakers = np.array([speaker_map[args.speaker_id]]) if model_config["multi_speaker"] else np.array([0]) 
