...
def train(rank, rank_jc, args, configs, batch_size, num_gpus):
    ...
    dataset = Dataset(
        "train_synthesize_{}.txt".format(level_tag), preprocess_config, model_config, train_config, sort=True, drop_last=True
    )
        ...
        for batchs in loader:
            ...
            for batch in batchs:
                ...
                with amp.autocast(args.use_amp):
                    ...
                    output, emphasize_output= model(*(batch[:]), step=step)
                    batch[9:11], output = output[-2:], output[:-2]

                    
                    losses = Loss(batch, output, emphasize_output, step=step)
                    total_loss = losses[0]
                ...
                if rank == rank_jc:
                    if step % log_step == 0:
                        ...
                        message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, CTC Loss: {:.4f}, Binarization Loss: {:.4f}, BCE Loss: {:.4f}".format(
                            *losses_
                        )
...
if __name__ == "__main__":
    ...
    if num_gpus > 1:
        ...
        train(0, 0, args, configs, batch_size, num_gpus)
