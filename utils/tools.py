...
def get_configs_of(dataset):
    config_dir = os.path.join("xxx", dataset)
...
def get_mask_from_lengths_audio(lengths, max_len=None):
...
def to_device(data, device):
    if len(data) == 18:
        ...
        if history_info is not None:
            ...
            if len(history_info) == 11:  
                ...
                history_waviemocap_embs = torch.from_numpy(history_waviemocap_embs).float().to(device)

                
                text_words_tod_embs = torch.from_numpy(text_words_tod_embs).float().to(device)
                history_text_words_tod_embs = torch.from_numpy(history_text_words_tod_embs).float().to(device)

                
                history_words_probabilitys = torch.from_numpy(history_words_probabilitys).to(device)
        ...
        if emphasis_data_batch is not None:
            (
                input_ids_batch,  
                token_map_batch,  
                token_label_batch,  
            ...
            input_ids_batch = torch.from_numpy(input_ids_batch).to(device)  
            token_map_batch = torch.from_numpy(token_map_batch).to(device)  
            token_label_batch = torch.from_numpy(token_label_batch).to(device)  
            words_len_batch = torch.from_numpy(np.array(words_len_batch)).to(device)  
            input_ids_masks = torch.from_numpy(np.array(input_ids_masks)).to(device)  
            punc_mask_batch = torch.from_numpy(np.array(punc_mask_batch)).to(device)  
    ...
    if len(data) == 11:
        (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, history_info, emphasis_data_batch, text_words_tod_emb_lens) = data
        ...
        return (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, history_info, emphasis_data_batch, text_words_tod_emb_lens)
...
def pad_1d_2(inputs, maxlen=0):
    
    inputs = [float(value) for value in inputs]
    
    inputs = torch.tensor(inputs)
    current_len = inputs.shape[0]

    if current_len >= maxlen:
        return inputs[:maxlen]

    
    pad_rows = max(0, maxlen - current_len)

    
    padded_array = np.pad(inputs, (0, pad_rows), 'constant', constant_values=0.0)

    return padded_array


def pad_2d_2(inputs, maxlen=0):
    current_len = inputs.shape[0]
    ...
    padded_array = np.pad(inputs, ((0, pad_rows), (0, 0)), 'constant', constant_values=(0, 0))
...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    if learn_alignment:
        ...
        attn_prior = attn_prior[0, :src_len, :mel_len].squeeze().detach().cpu().numpy() 
        attn_soft = attn_soft[0, 0, :mel_len, :src_len].detach().cpu().transpose(0, 1).numpy() 
        attn_hard = attn_hard[0, 0, :mel_len, :src_len].detach().cpu().transpose(0, 1).numpy() 
    ...
    with open(
        ...
        stats = json.load(f)
        stats = stats[f"pitch_{pitch_level_tag}"] + stats[f"energy_{energy_level_tag}"][:2] 
...
def synth_samples_savepred(targets, predictions, vocoder, model_config, preprocess_config, path, args, preds):
    ...
    for i in range(len(predictions[0])):
        ...
        if history_type == 'none':
            ...
            json_save_dir = os.path.join(
                path, str(args.restore_step), basename.split("_")[-1].strip("d"), "{}.json".format(basename))

            
            pred_dict = {
                "text": texts[i],
                "pred": ["{:.6f}".format(x) for x in preds[1][i].cpu().numpy().tolist()],
                "label": ["{:.6f}".format(x) for x in preds[0][i].cpu().numpy().tolist()]
            }
            with open(json_save_dir, 'w') as json_file:
    ...
    for i in range(len(basenames)):
        if history_type == 'none':
            os.makedirs((os.path.join(
                path, str(args.restore_step), basenames[i].split("_")[-1].strip("d"))), exist_ok=True)
            json_save_dir = os.path.join(
                path, str(args.restore_step), basename[i].split("_")[-1].strip("d"), "{}.json".format(basename))
